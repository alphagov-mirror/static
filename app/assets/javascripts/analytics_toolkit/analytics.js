;(function (global) {
  ...
  var Analytics = function (config) {
    this.stripPII = new GOVUK.StripPII()

    if (typeof config.stripDatePII !== 'undefined') {
      this.stripPII.stripDatePII = (config.stripDatePII === true)
    ...
    if (typeof config.stripPostcodePII !== 'undefined') {
      this.stripPII.stripPostcodePII = (config.stripPostcodePII === true)
      // remove the option so we don't pass it to other trackers - it's not
      // their concern
  ...
  Analytics.prototype.defaultPathForTrackPageview = function (location) {
    ...
    // were present
    return this.stripPII.stripPIIFromString(location.href.substring(location.origin.length).split('#')[0])
  }

  Analytics.prototype.trackPageview = function (path, title, options) {
    arguments[0] = arguments[0] || this.defaultPathForTrackPageview(window.location)
    if (arguments.length === 0) { arguments.length = 1 }
    this.sendToTrackers('trackPageview', this.stripPII.stripPII(arguments))
  }
  ...
  */
  Analytics.prototype.trackEvent = function (category, action, options) {
    this.sendToTrackers('trackEvent', this.stripPII.stripPII(arguments))
  }

  Analytics.prototype.trackShare = function (network, options) {
    this.sendToTrackers('trackSocial', this.stripPII.stripPII([network, 'share', global.location.pathname, options]))
  }
  ...
  Analytics.prototype.setDimension = function (index, value) {
    this.sendToTrackers('setDimension', this.stripPII.stripPII(arguments))
  }
